...
// Shortcut function to construct a SELECT query
Query.Select = function(from, obj) {
  return new Query('select', obj).from(from);
};

// Shortcut function to construct an UPDATE query
Query.Update = function(table, obj) {
  return new Query('update', obj).table(table);
};

// Shortcut function to construct a DELETE query
Query.Delete = function(from, obj) {
  return new Query('delete', obj).from(from);
};

// Shortcut function to construct an INSERT query
Query.Insert = function(into, obj) {
  return new Query('insert', obj).into(into);
};
 ...
 * * In SELECT queries represent the 'SELECT FROM <table>' (alias: 'from').
 * * In UPDATE queries represent the 'UPDATE <table>'.
 * * In INSERT queries represent the 'INSERT INTO <table>' (alias: 'into').
 * * In DELETE queries represent the 'DELETE FROM <table>' (alias: 'from').
 *
 * @example
...
Query.prototype.table = function(tableName) {
  this._table = utils.normalizeTable(tableName);
  return this;
};

/* Alias for 'table' used in SELECT and DELETE queries */
Query.prototype.from = function(tableName) {
  return this.table(tableName);
};

/* The table to store the results into.
 * In INSERT queries works as an alias for 'table' method */
Query.prototype.into = function(tableName) {
  if(this._command === 'select') {
    this._storeInto = utils.normalizeTable(tableName);
    return this;
  }
  else {
    return this.table(tableName);
...
Query.prototype.set = function(updates) {
  if(this._command !== 'update' && this._command !== 'insert')
    throw new Error('SET is only allowed in UPDATE and INSERT queries');
...
var commandOptions = {
  select: ['select', 'into', 'from', 'limit', 'skip', 'where', 'groupBy',
           'having', 'sort'],
  update: ['table', 'limit', 'set', 'where'],
  delete: ['from', 'limit', 'where'],
  insert: ['into', 'limit', 'set']
};
